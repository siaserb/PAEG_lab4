...
def decrypt(key_tuple, cipher):
    # deсrpyts each pair and adds the decrypted integer to list of plaintext integers
    plaintext = []
...
# Add these functions for signing and verification

def sign(private_key, message):
    # Hash the message for signing
    hashed_message = int.from_bytes(hashlib.sha256(message.encode()).digest(), 'big')

    # Choose a random k (0 < k < p-1)
    k = random.randint(1, private_key.p - 1)

    # Compute r = g^k mod p
    r = modexp(private_key.g, k, private_key.p)

    # Compute s = (hashed_message - private_key.x * r) * modinv(k, private_key.p-1) mod p-1
    s = (modexp(modinv(k, private_key.p - 1), hashed_message - private_key.x * r, private_key.p - 1)) % (
                private_key.p - 1)
...
def verify(public_key, message, signature):
    ...
    # Compute v1 = g^(hashed_message) mod p
    v1 = modexp(public_key.g, hashed_message, public_key.p)

    # Compute v2 = h^r * r^s mod p
    v2 = (modexp(public_key.h, r, public_key.p) * modexp(r, s, public_key.p)) % public_key.p

    # Verify if v1 is equal to v2
    return v1 == v2
...
verification_result = verify(pub, message, signature)
